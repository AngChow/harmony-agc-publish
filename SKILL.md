...
**若用户明确要求"提审"/"提交审核"，Step 4 成功后自动继续执行 Step 5，无需停下来等待二次确认。若用户只说"打包上传"而未提"提审"，到此为止。**
...
**该步骤会真正把版本提交到华为审核流程，影响线上**。

**执行规则**：
- 用户在当前会话中明确说"提审"/"提交审核"/"submit for review" → Step 4 成功后**自动执行** Step 5，一气呵成，不需要停下来二次确认。
- 用户只说"打包上传"而未提"提审" → **不执行** Step 5，到 Step 4 为止。

**AGC 包编译等待**：刚上传的 .app 包 AGC 需要编译处理（通常 1-2 分钟），期间 `app-submit` 接口会返回 HTTP 200 但 `ret.code=204144719`。脚本会自动轮询等待（每 15s 重试，最多 10 次），编译完成后自动提交，无需手动重跑。
