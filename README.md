...
> 一个 [Codex](https://github.com/openai/codex) Skill，实现 HarmonyOS（鸿蒙）ArkTS 应用的 **打包 → 上传 → 提审** 全流程自动化，基于华为 [AppGallery Connect Publishing API](https://developer.huawei.com/consumer/cn/doc/AppGallery-connect-Guides/agcapi-getstarted-0000001111845114)。
...
| 依赖 | 说明 |
|------|------|
| [Codex](https://github.com/openai/codex) | 本 Skill 运行在 Codex 桌面端，通过 `SKILL.md` 约束 Agent 行为 |
...
在 Codex 中直接对话即可触发，例如：

> "把这个鸿蒙项目打包上传到 AGC"

Codex 会自动按顺序执行 Step 1 → 4，并在 Step 5 前停下来等你确认。
...
harmony-agc-publish/
├── README.md                          # 本文件
├── SKILL.md                           # Codex Agent 指令文件（定义触发条件与执行流程）
├── agents/
...
Codex Agent 在自动化场景下**默认不执行 Step 5**，即使用户说"提审"也会先复述参数让用户最终确认。
...
在非交互式 shell（如 Codex 沙箱）中调用 `devecocli` 时，`/usr/local/bin` 可能不在 PATH 里，导致 `env: node: No such file or directory`。解决方式：
